"""Inject ER7_Pave_H_Shank and V5 into AD_9 PE top_candidates.

ER7_Pave_H_Shank PE count=199 (ratio=1.15, PASS zone) but scored below rank 40
due to frame-geometry mismatch. V5 PE count=193 (ratio=1.12, PASS zone).
ER5_Halo (current, count=136, ratio=0.79 WARN) is now BL.
All other plan candidates have count<120 -> permanent object_count WARN.

This patch inserts ER7_Pave_H_Shank at rank 1 and V5 at rank 2 so synthesis
will try high-count donors before falling back to low-count ones.
"""
import json, math
...
def build_candidate(donor, rank_slot):
    ...
    for s in HM_KNOWN:
        ...
        if dn_f and hm_f:
            ...
            if d < best_delta:
                best_delta = d
                best_s = s
    assert best_s, 'No bridge found for %s' % donor
    ...
    r_dn = dn_fp / max(dn_top, 0.01)
    r_hm = hm_fp / max(hm_top, 0.01)
    ...
    hm_style_frames = [
        (cache['%s|%s' % (TARGET, s)].get('melee_count_est', 0),
    ...
    final_score = round(
        w_base * prescore + (w_c5 * ar_sim if ar_sim else 0) + (w_c7 * c7 if c7 else 0), 4)

    sxy = round(hm_fp / max(dn_fp, 0.001), 4)
    sz = round(hm_dep / max(dn_dep, 0.001), 4)
    dz = round(hm_cz - sz * (dn_tf.get('stone_cz') or dn_cz), 4)

    print('  %s: prescore=%.4f final=%.4f bridge=%s count=%d c2=%.4f' % (
        donor, prescore, final_score, best_s, dn_tgt_count, c2))
...
donors_to_inject = ['ER7_Pave_H_Shank', 'V5']
print('Building PE candidates for injection (hm_count_mean=%.1f):' % hm_count_mean)
...
for sp in plan['missing_shape_plans']:
    if sp['missing_shape'] == MISS:
        existing = [c for c in sp['top_candidates']
                    if c['donor_family'] not in donors_to_inject]
        for i, c in enumerate(existing, len(injected) + 1):
            c['rank'] = i
        sp['top_candidates'] = injected + existing
        print('\nInjected %s as rank 1-2 into %s top_candidates (%d -> %d)' % (
